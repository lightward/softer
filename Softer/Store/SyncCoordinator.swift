...
actor SyncCoordinator {
    ...
    // MARK: - Invitations (Public Database)

    private static let invitationRecordType = "Invitation"

    /// Create invitations in the public database for share recipients.
    func createInvitations(
        ...
        for toEmail in emails {
            ...
            record["toEmail"] = toEmail.lowercased() as CKRecordValue  // Normalize to lowercase
            record["fromUserRecordID"] = fromUserRecordID as CKRecordValue
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            operation.modifyRecordsResultBlock = { result in
                switch result {
                case .success:
                    print("SyncCoordinator: Created \(records.count) invitation(s) for emails: \(emails)")
                    continuation.resume()
                case .failure(let error):
                    print("SyncCoordinator: Failed to create invitations: \(error)")
                    continuation.resume(throwing: error)
                }
            }
    ...
    /// Fetch pending invitations for the current user from public database.
    /// Discovers the user's iCloud email(s) and queries for invitations addressed to them.
    func fetchInvitations() async throws -> [Invitation] {
        // Discover our own email(s) from iCloud identity
        let myEmails = await discoverOwnEmails()
        guard !myEmails.isEmpty else {
            print("SyncCoordinator: Could not discover own emails for fetching invitations")
            return []
        }

        print("SyncCoordinator: Fetching invitations for emails: \(myEmails)")

        let publicDatabase = container.publicCloudDatabase

        // Query for invitations to any of our emails
        let emailPredicates = myEmails.map { NSPredicate(format: "toEmail == %@", $0.lowercased()) }
        let predicate = NSCompoundPredicate(orPredicateWithSubpredicates: emailPredicates)
        ...
        do {
            ...
            for (_, result) in results {
                if case .success(let record) = result {
                    if let invitation = Self.invitation(from: record) {
                        invitations.append(invitation)
                    }
                }
            }

            print("SyncCoordinator: Fetched \(invitations.count) invitation(s)")
            return invitations
        } catch {
            print("SyncCoordinator: Failed to fetch invitations: \(error)")
            throw error
        }
    }

    /// Discover the current user's iCloud email addresses.
    /// Tries CloudKit discoverability first, falls back to stored email.
    private nonisolated func discoverOwnEmails() async -> [String] {
        // First try stored email (set by user or previous discovery)
        if let storedEmail = UserDefaults.standard.string(forKey: "SofterUserEmail") {
            print("SyncCoordinator: Using stored email: \(storedEmail)")
            return [storedEmail]
        }

        // TEMPORARY: Check for well-known test accounts
        // TODO: Replace with proper email entry UI
        do {
            let userRecordID = try await container.userRecordID()
            let recordName = userRecordID.recordName
            print("SyncCoordinator: User record ID: \(recordName)")

            // Known test account mappings (temporary workaround)
            let knownAccounts: [String: String] = [
                "_403cad05dd0fad9345c2e861b75b3045": "abrahamplopez@gmail.com"  // Abe's iPad
            ]
            if let email = knownAccounts[recordName] {
                print("SyncCoordinator: Using known account email: \(email)")
                UserDefaults.standard.set(email, forKey: "SofterUserEmail")
                return [email]
            }
        } catch {
            print("SyncCoordinator: Failed to get user record ID: \(error)")
        }

        // Try CloudKit discoverability
        do {
            // Check current permission status first
            let currentStatus = try await container.applicationPermissionStatus(for: .userDiscoverability)
            print("SyncCoordinator: Current discoverability status: \(currentStatus.rawValue)")
            ...
            if currentStatus == .initialState {
                ...
                print("SyncCoordinator: Requested permission, new status: \(newStatus.rawValue)")
                guard newStatus == .granted else {
                    print("SyncCoordinator: User discoverability permission not granted")
                    return []
                }
            } else if currentStatus != .granted {
                print("SyncCoordinator: User discoverability not granted (status: \(currentStatus.rawValue))")
                return []
            }
            ...
            if let email = identity?.lookupInfo?.emailAddress {
                ...
                UserDefaults.standard.set(email, forKey: "SofterUserEmail")
                print("SyncCoordinator: Discovered and stored email: \(email)")
            }

            if emails.isEmpty {
                print("SyncCoordinator: Identity found but no email in lookupInfo")
                if let identity = identity {
                    print("SyncCoordinator: Identity details - lookupInfo: \(String(describing: identity.lookupInfo))")
                }
            }

            return emails
        } catch {
            print("SyncCoordinator: Failed to discover own identity: \(error)")
            return []
        }
    }

    /// Delete an invitation after it's been accepted.
    func deleteInvitation(id: String) async throws {
        ...
        do {
            ...
            print("SyncCoordinator: Deleted invitation \(id)")
        } catch {
            print("SyncCoordinator: Failed to delete invitation: \(error)")
            throw error
        }
    }

    /// Accept a share using the URL from an invitation.
    func acceptShareFromInvitation(_ invitation: Invitation) async throws -> String? {
        guard let url = URL(string: invitation.shareURL) else {
            print("SyncCoordinator: Invalid share URL in invitation")
            return nil
        }

        let roomID = try await acceptShare(from: url)

        // Delete the invitation after successful acceptance
        try? await deleteInvitation(id: invitation.id)

        return roomID
    }

    private nonisolated func getLocalUserRecordID() async -> String? {
        do {
    ...
    private static func invitation(from record: CKRecord) -> Invitation? {
        guard let toEmail = record["toEmail"] as? String,
              let fromUserRecordID = record["fromUserRecordID"] as? String,
